...
import { queueDriveImport } from './lib/api'
...
export default function App() {
  ...
  if (view === 'admin') return <Admin event={event} user={user} onBack={() => setView('landing')} onLogout={logout} onCreate={() => setShowCreate(true)} onRefresh={() => setEvent((current) => ({ ...current, photos: [...current.photos] }))} onCreateEvent={(title, date, location) => { setEvent((current) => ({ ...current, title, date, location, status: 'draft', photos: [] })); setShowCreate(false) }} showCreate={showCreate} onCloseCreate={() => setShowCreate(false)} />
...
function Admin({ event, user, onBack, onLogout, onCreate, onRefresh, onCreateEvent, showCreate, onCloseCreate }: { event: GalleryEvent; user: SessionUser | null; onBack: () => void; onLogout: () => void; onCreate: () => void; onRefresh: () => void; onCreateEvent: (title: string, date: string, location: string) => void; showCreate: boolean; onCloseCreate: () => void }) {
...
function CreateEventModal({ onClose, onSubmit }: { onClose: () => void; onSubmit: (title: string, date: string, location: string) => void }) {
  const [title, setTitle] = useState('')
  const [date, setDate] = useState('')
  const [location, setLocation] = useState('')
  return <div className="modal-backdrop"><form className="modal" onSubmit={(e) => { e.preventDefault(); onSubmit(title, date || 'Draft event', location || 'Location to be added') }}><button type="button" className="modal-close" onClick={onClose}><X size={17} /></button><p className="eyebrow">A new story</p><h2>Create event</h2><label>Event title<input required value={title} onChange={(e) => setTitle(e.target.value)} placeholder="A day in the Aravallis" /></label><label>Date<input value={date} onChange={(e) => setDate(e.target.value)} placeholder="18 February 2025" /></label><label>Location<input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="Udaipur, Rajasthan" /></label><button className="button button-dark" type="submit">Create draft <ArrowUpRight size={15} /></button></form></div>
}
